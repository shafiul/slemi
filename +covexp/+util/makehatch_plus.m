...
%covexp.util.makehatch_plus Predefined hatch patterns
%
...
%
%  covexp.util.makehatch_plus(HATCH,N,M) returns a matrix with the hatch pattern for HATCH
...
switch (hatch)
  case '\'
    ...
    A = A | abs(B+n) <= m/2;
  case '/'
    A = fliplr(covexp.util.makehatch_plus('\',n,m));
  case '|'
    A=zeros(n);
    A(:,1:m) = 1;
  case '-'
    A = covexp.util.makehatch_plus('|',n,m);
    A = A';
  case '+'
    A = covexp.util.makehatch_plus('|',n,m);
    A = A | A';
