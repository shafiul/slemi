...
%  thickness can be modified using a direct call to covexp.util.makehatch_plus using
%  the following syntax: covexp.util.makehatch_plus('HHn',m) where;
%       HH 	the hatch character written twice, '//', '\\', '||', '--', '++'
%       n   integer number for thickness
%       m   integer number for the matrix size (n<=m)
% Ex. covexp.util.makehatch_plus('\\4',9)
%
...
%       bar(rand(3,3));
%       im_hatch = applyhatch_plusC(gcf,{covexp.util.makehatch_plus('\',6),1-covexp.util.makehatch_plus('\',6),covexp.util.makehatch_plus('\',1)},'ggg');%
%
% Example 5: using variable thickness hatches
%       bar(rand(3,3));
%       im_hatch = applyhatch_plusC(gcf,{covexp.util.makehatch_plus('\',9),covexp.util.makehatch_plus('\\4',9),covexp.util.makehatch_plus('\\8',9)},'rgb');%
%
...
while ~isempty(colorind)
  ...
  elseif isa(patterns,'char')
    pattern = covexp.util.makehatch_plus(patterns(pati),6*hatchsc);
  else
    pattern = patterns;
  end
